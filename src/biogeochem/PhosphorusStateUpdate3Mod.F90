...
!hh!        do j = 1, nlevdecomp
!hh!          do fc = 1,num_soilc
!hh!            c = filter_soilc(fc)
!hh!            col_ps%primp_vr(c,j)   = col_ps%primp_vr(c,j) - col_pf%primp_to_labilep_vr(c,j) *dt &
!hh!                 + col_pf%pdep_to_sminp(c)*dt * pdep_prof(c,j)
!hh!          end do
!hh!        enddo
!hh!      else
