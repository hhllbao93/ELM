...
contains
  ...
  subroutine FireInit( bounds )
    ...
    !-----------------------------------------------------------------------
#ifndef CPL_BYPASS
    call hdm_init(bounds, NLFilename)
 ...
 !-----------------------------------------------------------------------
 subroutine hdm_init( bounds, NLFilename)
   !
   ! !DESCRIPTION:
   ...
   if (masterproc) then
      ...
      write(iulog,*) '  stream_fldFileName_popdens = ',trim(stream_fldFileName_popdens)
      write(iulog,*) '  stream_meshfile_popdens    = ',trim(stream_meshfile_popdens)
      write(iulog,*) '  stream_varnames            = ','hdm'
      write(iulog,*) '  time interp algo           = ',trim(popdens_tintalgo)
      write(iulog,*) '  mapping interp algo        = ',trim(popdensmapalgo)
      write(iulog,*) ' '
   endif

    ! Initialize the cdeps data type sdat
    call shr_strdata_init_from_inline(sdat_hdm,                 &
         my_task             = iam,                                  &
         logunit             = iulog,                                &
         compname            = 'LND',                                &
...
subroutine hdm_interp(bounds)
  ...
  !-----------------------------------------------------------------------
   ...
   call shr_strdata_advance(sdat_hdm, ymd=mcdate, tod=sec, logunit=iulog, istr='hdmdyn', rc=rc)
   if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) then
      call ESMF_Finalize(endflag=ESMF_END_ABORT)
   end if

   ! Get pointer for stream data that is time and spatially interpolate to model time and grid
   call dshr_fldbun_getFldPtr(sdat_hdm%pstrm(1)%fldbun_model, 'hdm', fldptr1=dataptr1d, rc=rc)
   if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) then
      call ESMF_Finalize(endflag=ESMF_END_ABORT)
   end if
...
subroutine lnfm_init( bounds, NLFilename )
  ...
  !-----------------------------------------------------------------------
   ...
   if (masterproc) then
      ...
      write(iulog,*) '  stream_fldFileName_lightng = ',trim(stream_fldFileName_lightng)
      write(iulog,*) '  stream_meshfile            = ',trim(stream_meshfile_lightng)
      write(iulog,*) '  stream_varnames            = ','lnfm' 
      write(iulog,*) '  time interp algo           = ',trim(lightng_tintalgo)
      write(iulog,*) '  mapping interp algo        = ',trim(lightngmapalgo)
      write(iulog,*) ' '
   endif

    ! Initialize the cdeps data type sdat
    call shr_strdata_init_from_inline(sdat_lnfm,                &
         my_task             = iam,                                  &
         logunit             = iulog,                                &
         compname            = 'LND',                                &
         model_clock         = model_clock,                          &
...
subroutine lnfm_interp(bounds )
  ...
  !-----------------------------------------------------------------------
   ...
   call get_curr_date(year, mon, day, sec)
   mcdate = year*10000 + mon*100 + day
   call shr_strdata_advance(sdat_lnfm, ymd=mcdate, tod=sec, logunit=iulog, istr='lnfmdyn', rc=rc)
   if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) then
      call ESMF_Finalize(endflag=ESMF_END_ABORT)
   end if

   ! Get pointer for stream data that is time and spatially interpolate to model time and grid
   call dshr_fldbun_getFldPtr(sdat_lnfm%pstrm(1)%fldbun_model, 'lnfm', fldptr1=dataptr1d, rc=rc)
   if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) then
      call ESMF_Finalize(endflag=ESMF_END_ABORT)
   end if
