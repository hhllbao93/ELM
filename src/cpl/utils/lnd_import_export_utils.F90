...
!=============================================================================
  ...
  subroutine derive_quantities( bounds, atm2lnd_vars, &
    ...
    use elm_varcon, only: rair, o2_molar_const
    ...
    real(r8) :: es          ! saturation specific humidity (kg/kg) !hh!
    real(r8) :: esdT        ! saturation specific humidity (kg/kg) !hh!
    real(r8) :: qsdT        ! saturation specific humidity (kg/kg) !hh!
    ...
    do g = begg, endg
       ...
       atm2lnd_vars%forc_rain_not_downscaled_grc(g)  = forc_rainc(g) + forc_rainl(g)
       atm2lnd_vars%forc_snow_not_downscaled_grc(g)  = forc_snowc(g) + forc_snowl(g)

       call QSat(forc_t, forc_pbot, es, esdT, qsat_kg_kg, qsdT)

       atm2lnd_vars%forc_rh_grc(g) = 100.0_r8*(forc_q / qsat_kg_kg)
    end do
  ...
  subroutine check_for_errors( bounds, atm2lnd_vars)
    ...
    do g = begg, endg
       if ( atm2lnd_vars%forc_lwrad_not_downscaled_grc(g) <= 0.0_r8 ) then
          call shr_sys_abort( subname//&
               ' ERROR: Longwave down sent from the atmosphere model is negative or zero' )
       end if
       if ( (atm2lnd_vars%forc_solad_grc(g,1) < 0.0_r8) .or. &
            (atm2lnd_vars%forc_solad_grc(g,2) < 0.0_r8) .or. &
            (atm2lnd_vars%forc_solai_grc(g,1) < 0.0_r8) .or. &
            (atm2lnd_vars%forc_solai_grc(g,2) < 0.0_r8) ) then
          call shr_sys_abort( subname//&
               ' ERROR: One of the solar fields (indirect/diffuse, vis or near-IR)'// &
               ' from the atmosphere model is negative or zero' )
       end if
       if ( atm2lnd_vars%forc_q_not_downscaled_grc(g) < 0.0_r8 )then
          call shr_sys_abort( subname//&
