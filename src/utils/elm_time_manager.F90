...
!hh!    ! pflotran: nsstep
!hh!    ! save the first restart 'nstep'
!hh!    nsstep = get_nstep()
!hh!
!hh!    ! Calculate ending time step
!hh!
!hh!    call calc_nestep( )
...
!hh!  function get_curr_calday(offset)
...
!!!!!!!!!!!!!! END HACK TO ENABLE Gregorian CALENDAR WITH SHR_ORB !!!!!!!!!!!!!!!!!!!!!!!!
  ...
  integer function get_prev_days_per_year()
    ...
    get_prev_days_per_year = get_days_per_year(offset = -dtime)
...
!hh!    call check_timemgr_initialized(sub)
    if ( .not. check_timemgr_initialized(sub) ) return
...
!hh!    call check_timemgr_initialized(subname)
...
!hh!  subroutine check_timemgr_initialized(caller)
...
!hh!  end subroutine check_timemgr_initialized
