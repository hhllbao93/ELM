...
!hh!    call get_elmlevel_gsmap(elmlevel=trim(elmlevel), gsmap=gsmap)
!hh!    call mct_gsMap_orderedPoints(gsmap, iam, gsmap_ordered)
!hh!    GetGlobalIndex = gsmap_ordered(decomp_index - beg_index + 1)
!hh!    deallocate(gsmap_ordered)

    call get_elmlevel_gsmap(elmlevel_name=trim(elmlevel), gsmap=gsmap)
    GetGlobalIndex = gsmap(decomp_index - beg_index + 1)
  ...
  function GetGlobalIndexArray(decomp_index, bounds1, bounds2, elmlevel)
    ...
    ! Local Variables:
    type(bounds_type)             :: bounds_proc   ! processor bounds
!hh!    type(mct_gsMap),pointer       :: gsmap         ! global seg map
    integer , pointer 		  :: gsmap(:)
    integer                       :: beg_index     ! beginning proc index for elmlevel
    integer                       :: i
    !----------------------------------------------------------------
...
!hh!    call get_elmlevel_gsmap(elmlevel=trim(elmlevel), gsmap=gsmap)
!hh!    call mct_gsMap_orderedPoints(gsmap, iam, gsmap_ordered)
!hh!    do i=bounds1,bounds2
!hh!       GetGlobalIndexArray(i) = gsmap_ordered(decomp_index(i) - beg_index + 1)
!hh!    enddo
!hh!    deallocate(gsmap_ordered)

    call get_elmlevel_gsmap(elmlevel_name=trim(elmlevel), gsmap=gsmap)
    do i=bounds1,bounds2
       GetGlobalIndexArray(i) = gsmap(decomp_index(i) - beg_index + 1)
    enddo
