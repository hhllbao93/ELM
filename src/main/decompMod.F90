module decompMod
  ...
  integer, public, pointer :: gsMap_lnd_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_gce_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_top_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_lun_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_col_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_patch_gdc2glo(:)   => null()
  integer, public, pointer :: gsMap_cohort_gdc2glo(:)   => null()
  !------------------------------------------------------------------------------

contains

  !-----------------------------------------------------------------------
  pure function get_beg(bounds, elmlevel) result(beg_index)
    !
    ! !DESCRIPTION:
    ! Get beginning bounds for a given subgrid level
    !
    ! elmlevel should be one of the constants defined in this module:
    ! BOUNDS_SUBGRID_GRIDCELL, BOUNDS_SUBGRID_LANDUNIT, etc.
    !
    ! Returns -1 for invalid elmlevel (does not abort in this case, in order to keep
    ! this function pure).
    !
    ! !USES:
    !
  ...
  !-----------------------------------------------------------------------
  pure function get_end(bounds, elmlevel) result(end_index)
    !
    ! !DESCRIPTION:
  ...
  !------------------------------------------------------------------------------
   subroutine get_clump_bounds_new (n, bounds)
    ...
    bounds%begt      = clumps(cid)%begt - procinfo%begt + 1
    bounds%endt      = clumps(cid)%endt - procinfo%begt + 1
   ...
   subroutine get_proc_bounds_new (bounds)
    ...
    bounds%begt = 1 
    bounds%endt = procinfo%endt - procinfo%begt + 1
    bounds%begg = 1
   ...
   !-----------------------------------------------------------------------
   integer function get_elmlevel_gsize (elmlevel_name)
     !
     ! !DESCRIPTION:
     ...
     !
     ! !ARGUMENTS:
     character(len=*), intent(in) :: elmlevel_name    !type of clm 1d array
     !-----------------------------------------------------------------------

     select case (elmlevel_name)
     case(grlnd)
     ...
     case default
        write(iulog,*) 'get_elmlevel_gsize does not match elmlevel type: ', trim(elmlevel_name)
        call shr_sys_abort()
     end select
   ...
   !-----------------------------------------------------------------------
   subroutine get_elmlevel_gsmap(elmlevel_name, gsmap)
     !
     ! !DESCRIPTION:
     ! Compute arguments for gatherv, scatterv for vectors
     !
     ! !ARGUMENTS:
     character(len=*), intent(in) :: elmlevel_name     ! type of input data
     integer         , pointer    :: gsmap(:)
!hh!    type(mct_gsmap) , pointer    :: gsmap
    !----------------------------------------------------------------------
    ...
    case default
       write(iulog,*) 'get_elmlevel_gsmap: Invalid expansion character: ',trim(elmlevel_name)
       call shr_sys_abort()
    end select
