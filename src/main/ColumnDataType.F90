...
!hh!  use MathfuncMod     , only : dot_sum
...
!hh!          call restartvar(ncid=ncid, flag=flag, varname='totblgc', xtype=ncd_double,  &
!hh!               dim1name='column', long_name='', units='', &
!hh!               interpinic_flag='interp', readvar=readvar, data=this%totblgc)
!hh!
!hh!          call restartvar(ncid=ncid, flag=flag, varname='cwdc', xtype=ncd_double,  &
!hh!               dim1name='column', long_name='', units='', &
!hh!               interpinic_flag='interp', readvar=readvar, data=this%cwdc)
!hh!       endif
...
!hh!       endif ! .not. is_active_betr_bgc
       ! still in C12 block
...
!hh!    if ( (.not. is_active_betr_bgc           ) .and. &
!hh!         (.not. (use_pflotran .and. pf_cmode))) then
...
!hh!    elseif (is_active_betr_bgc) then
!hh!
!hh!       do fc = 1, num_soilc
!hh!          c = filter_soilc(fc)
!hh!          this%hr(c) = dot_sum(this%hr_vr(c,1:nlevdecomp),dzsoi_decomp(1:nlevdecomp))
!hh!       enddo
    endif
...
!hh!    end if ! .not. is_active_betr_bgc
