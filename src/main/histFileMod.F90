...
#include "shr_assert.h"
  ...
  type field_info
     ...
     character(len=scale_type_strlen) :: p2c_scale_type       ! scale factor when averaging pft to column
     ...
     character(len=scale_type_strlen) :: t2g_scale_type       ! scale factor when averaging topounit to gridcell
...
!hh!    ! Ensure there are no "holes" in tape specification, i.e. empty tapes.
!hh!    ! Enabling holes should not be difficult if necessary.
!hh!
!hh!    do t = 1,ntapes
!hh!       if (tape(t)%nflds  ==  0) then
!hh!          write(iulog,*) trim(subname),' ERROR: Tape ',t,' is empty'
!hh!          call endrun(msg=errMsg(__FILE__, __LINE__))
!hh!       end if
!hh!    end do
!hh!
!hh!    ! Check that the number of history files declared does not exceed
!hh!    ! the maximum allowed.
!hh!
!hh!    if (ntapes > max_tapes) then
!hh!       write(iulog,*) trim(subname),' ERROR: Too many history files declared, max_tapes=',max_tapes
!hh!       call endrun(msg=errMsg(__FILE__, __LINE__))
!hh!    end if
...
!$OMP END PARALLEL DO
  ...
  subroutine hist_update_hbuf_field_1d (t, f, bounds)
    ...
    character(len=scale_type_strlen)  :: t2g_scale_type ! scale type for subgrid averaging of topounits to gridcells
...
!hh!             ! Define 3D time-constant field variables only to first primary tape
!hh!             if ( do_3Dtconst .and. t == 1 ) then
...
!hh!          ! Write 3D time constant history variables only to first primary tape
!hh!          if ( do_3Dtconst .and. t == 1 .and. tape(t)%ntimes == 1 )then
...
!hh!               units="A=Average, X=Maximum, M=Minimum, I=Instantaneous", &
!hh!               dim1name='len1', dim2name='max_nflds' )
...
!hh!          call ncd_io('locfnh',  locfnh(t),  'write', ncid, nt=t)
!hh!          call ncd_io('locfnhr', locfnhr(t), 'write', ncid, nt=t)
       end do
...
!hh!       if ( is_restart() .and. ntapes_onfile /= ntapes )then
       if (is_restart()) then
...
!hh!          if ( is_restart() .and. ntapes > 0 )then
          if (ntapes > 0) then
  ...
  subroutine hist_addfld1d (fname, units, avgflag, long_name, type1d_out, standard_name, &
    ...
    character(len=scale_type_strlen) :: scale_type_t2g ! scale type for subgrid averaging of topounits to gridcells
