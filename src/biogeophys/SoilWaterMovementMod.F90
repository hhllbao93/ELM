...
!hh!  use ExternalModelConstants     , only : EM_VSFM_SOIL_HYDRO_STAGE
!hh!  use ExternalModelConstants     , only : EM_ID_VSFM
!hh!  use ExternalModelInterfaceMod  , only : EMI_Driver
...
contains
  ...
  subroutine SoilWater(bounds, num_hydrologyc, filter_hydrologyc, &
    ...
    !USES
      !$acc routine seq
!hh!    use elm_varctl                 , only : use_betr
...
!hh!    case (vsfm)
!hh!#ifdef USE_PETSC_LIB
!hh!#ifndef _OPENACC
!hh!       call Prepare_Data_for_EM_VSFM_Driver(bounds, num_hydrologyc, filter_hydrologyc, &
!hh!            soilhydrology_vars, soilstate_vars, &
!hh!            waterflux_vars, waterstate_vars, temperature_vars)
!hh!
!hh!       call EMI_Driver(EM_ID_VSFM, EM_VSFM_SOIL_HYDRO_STAGE, dt = get_step_size()*1.0_r8, &
!hh!            number_step = get_nstep(), &
!hh!            clump_rank  = bounds%clump_index, &
!hh!            num_hydrologyc=num_hydrologyc, filter_hydrologyc=filter_hydrologyc, &
!hh!            soilhydrology_vars=soilhydrology_vars, soilstate_vars=soilstate_vars, &
!hh!            waterflux_vars=waterflux_vars, waterstate_vars=waterstate_vars, &
!hh!            temperature_vars=temperature_vars)
!hh!#endif
!hh!#endif
    case default
#ifndef _OPENACC
...
!hh!    if(use_betr)then
!hh!    !a work around of the negative liquid water embarrassment, which is
!hh!    !critical for a meaningufl tracer transport in betr. Jinyun Tang, Jan 14, 2015
!hh!
!hh!    do fc = 1, num_hydrologyc
!hh!       c = filter_hydrologyc(fc)
!hh!       nlevbed = nlev2bed(c)
!hh!       do j = 1, nlevbed-1
!hh!          if (h2osoi_liq(c,j) < 0._r8) then
!hh!             xs(c) = watmin - h2osoi_liq(c,j)
!hh!          else
!hh!             xs(c) = 0._r8
!hh!          end if
!hh!          h2osoi_liq(c,j  ) = h2osoi_liq(c,j  ) + xs(c)
!hh!          h2osoi_liq(c,j+1) = h2osoi_liq(c,j+1) - xs(c)
!hh!       end do
!hh!    end do
!hh!
!hh!    do fc = 1, num_hydrologyc
!hh!       c = filter_hydrologyc(fc)
!hh!       j = nlev2bed(c)
!hh!       if (h2osoi_liq(c,j) < watmin) then
!hh!          xs(c) = watmin-h2osoi_liq(c,j)
!hh!        else
!hh!          xs(c) = 0._r8
!hh!       end if
...
!hh!    end do
!hh!
!hh!    !update volumetric soil moisture for bgc calculation
!hh!    do fc = 1, num_hydrologyc
!hh!       c = filter_hydrologyc(fc)
!hh!       nlevbed = nlev2bed(c)
!hh!       do j = 1, nlevbed
!hh!          h2osoi_vol(c,j) = h2osoi_liq(c,j)/(dz(c,j)*denh2o) &
!hh!                            + h2osoi_ice(c,j)/(dz(c,j)*denice)
!hh!       enddo
!hh!    enddo
!hh!    endif
  end associate
