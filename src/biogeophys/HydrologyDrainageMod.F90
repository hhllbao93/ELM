...
!hh!  use elm_instMod   , only : ep_betr
...
!hh!    use elm_varctl       , only : glc_snow_persistence_max_days, use_vichydro, use_betr
...
!hh!      if (use_betr) then
!hh!        call ep_betr%BeTRSetBiophysForcing(bounds, col_pp, veg_pp, 1, nlevsoi, waterstate_vars=col_ws, &
!hh!          waterflux_vars=col_wf)
!hh!        call ep_betr%DiagDrainWaterFlux(num_hydrologyc, filter_hydrologyc)
