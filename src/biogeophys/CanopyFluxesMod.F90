...
!hh!    use FrictionVelocityMod, only : FrictionVelocity, MoninObukIni
...
!hh!         call MoninObukIni(ur(p), thv(c), dthv(p), zldis(p), z0mv(p), um(p), obu(p))
         call frictionvel_vars%MoninObukIni(ur(p), thv(c), dthv(p), zldis(p), z0mv(p), um(p), obu(p))
...
!hh!               tau(p) = forc_rho(t)*wind_speed_adj(p)/ram1(p)
